...
const navItems = [
  ...
  { name: 'Service Manager', icon: Settings2, href: '/admin/services', isAdmin: true },
  { name: 'Form Designer', icon: ClipboardList, href: '/admin/form-builder', isAdmin: true },
  { name: 'Settings', icon: Settings, href: '/settings', isAdmin: true },
];
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex font-sans text-slate-900 selection:bg-teal-100 selection:text-teal-900">
      ...
      >
        ...
        <nav className="p-6 space-y-1.5 flex-1 overflow-y-auto custom-scrollbar">
          {navItems.map((item) => {
            ...
            if (item.isAdmin && role !== 'Admin') return null;
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3.5 relative z-10">
                  ...
                  <span className="font-bold text-sm tracking-wide text-left">{item.name}</span>
                </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
          ...
          <div className="flex items-center gap-2 lg:gap-6">
            <div className="flex items-center gap-1">
              <button className="p-3 text-slate-500 hover:bg-slate-100 rounded-2xl transition-all relative group">
                ...
                <span className="absolute top-2.5 right-2.5 w-2 h-2 bg-red-500 rounded-full border-2 border-white ring- ring-red-500/20"></span>
              </button>
            </div>
            ...
            <div className="relative">
              ...
              >
                <div className="w-10 h-10 bg-gradient-to-br from-slate-800 to-slate-900 text-teal-400 rounded-xl flex items-center justify-center font-black shadow-lg shadow-slate-200 group-active:scale-95 transition-transform shrink-0">
                  {user?.email?.[0].toUpperCase() || 'U'}
                </div>
                <div className="text-left hidden sm:block min-w-0">
                  <p className="text-xs font-black text-slate-900 uppercase tracking-tight truncate">{role || 'User'}</p>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest truncate max-w-[100px]">{user?.email}</p>
                </div>
                <ChevronDown size={14} className={cn("text-slate-400 transition-transform duration-300 shrink-0", isUserDropdownOpen && "rotate-180")} />
              </button>
                ...
                {isUserDropdownOpen && (
                  ...
                  >
                    <div className="p-3 mb-2 bg-slate-50 rounded-xl">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.15em] mb-1">Clinic Access</p>
                      <p className="text-xs font-bold text-slate-900 truncate">{user?.email}</p>
                    </div>
                    <button className="flex items-center gap-3 w-full p-2.5 text-sm font-bold text-slate-600 hover:bg-slate-50 hover:text-slate-900 rounded-xl transition-colors text-left">
                    ...
                    <button 
                      ...
                      className="flex items-center gap-3 w-full p-2.5 text-sm font-bold text-red-600 hover:bg-red-50 rounded-xl transition-colors text-left"
                    >
                      <LogOut size={18} />
                      Lock Session
                    </button>
        ...
        {/* Page Content */}
        <main className="flex-1 overflow-x-hidden overflow-y-auto p-4 sm:p-6 lg:p-10 bg-[#FBFBFE] custom-scrollbar relative">
