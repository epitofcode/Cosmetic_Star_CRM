  ...
  return (
    <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-xl shadow-slate-200/40 space-y-6">
      ...
      {photos.length > 0 ? (
        ...
        <div 
          onClick={() => fileInputRef.current?.click()}
          className="h-48 border-2 border-dashed border-slate-100 rounded-3xl bg-slate-50/50 flex flex-col items-center justify-center gap-3 cursor-pointer group hover:bg-slate-50 hover:border-teal-500/30 transition-all"
        >
          <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-slate-300 group-hover:text-teal-500 shadow-sm border border-slate-100 transition-colors">
            <ImageIcon size={24} />
          </div>
  ...
  return (
    <div className="space-y-10">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h2 className="text-2xl font-black text-slate-900 tracking-tight">Clinical Media Manager</h2>
          <p className="text-slate-500 font-bold uppercase tracking-widest text-[10px] mt-1">High-Fidelity Procedure Charting</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="bg-white border border-slate-200 text-slate-400 px-4 py-2 rounded-xl font-bold text-[10px] uppercase tracking-widest flex items-center gap-2 shadow-sm">
          ...
          <button 
            ...
            className="bg-teal-600 hover:bg-teal-500 text-white px-6 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all shadow-lg shadow-teal-600/20 disabled:opacity-50 disabled:bg-slate-800 disabled:shadow-none"
          >
            Synchronize Gallery
          </button>
        </div>
      ...
      {/* Compliance Footer */}
      <div className="bg-slate-900 rounded-3xl p-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-white">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400 font-medium mt-1 italic">Images will be stored in the GDPR-compliant Clinical Vault.</p>
          </div>
        </div>
